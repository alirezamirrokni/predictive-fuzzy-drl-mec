...
def evaluate_lstm(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    x, y = build_sequence_windows(trace, int(config.get("lookback_window", 10)), int(config.get("prediction_horizon", 1)), target_size=target_size)
    ...
    result = {"model": "lstm", "checkpoint": str(args.checkpoint_path), "cache": str(cache_path), "metrics": regression_metrics(y, pred), "epoch": int(checkpoint.get("epoch", -1))}
    return result
...
def evaluate_gnn(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    x, y = build_graph_windows(trace, int(config.get("lookback_window", 10)), int(config.get("prediction_horizon", 1)), target_columns=tuple(config.get("target_columns", [0, 1])))
    device = torch.device(args.device if args.device else ("cuda" if torch.cuda.is_available() else "cpu"))
    adjacency = torch.as_tensor(build_distance_adjacency(positions, int(config.get("k_neighbors", 5))), dtype=torch.float32, device=device)
    ...
    result = {"model": "gnn", "checkpoint": str(args.checkpoint_path), "cache": str(cache_path), "metrics": regression_metrics(y, pred), "epoch": int(checkpoint.get("epoch", -1))}
    return result
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--result-path", default="data/results/predictor_eval.json")
